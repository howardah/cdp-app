...
export interface FileConstraint {
  ...
  min?: number; max?: number; inputId?: string;
}
export interface InputDefinition {
  id: string; label: string; description: string; fileTypes: CdpFileType[];
  minItems: number; maxItems: number | null; ordered: boolean; constraints: FileConstraint[];
}
export type CliBinding =
  | { kind: "positional" }
  | { kind: "option"; flag: string; join: "concatenated" | "separate" }
  | { kind: "booleanFlag"; flag: string };
export interface ParameterBase { id: string; label: string; description: string; required: boolean; advanced: boolean; unit?: string; cli: CliBinding; }
export type ParameterDefinition =
  | (ParameterBase & { kind: "number" | "integer"; default: number; min?: number; max?: number; step?: number })
  | (ParameterBase & { kind: "choice"; default: string; choices: { value: string; label: string; cliValue: string }[] })
  | (ParameterBase & { kind: "flag"; default: boolean })
  | (ParameterBase & { kind: "file"; fileTypes: CdpFileType[] })
  | (ParameterBase & { kind: "numberOrBreakpoint"; default: { kind: "number"; value: number }; min?: number; max?: number; step?: number; breakpointType: "time-value-pairs" });
export type ValueRef =
  | { source: "parameter"; id: string }
  | { source: "inputMetadata"; inputId: string; property: "duration" | "channels" }
  | { source: "literal"; value: number };
export type Constraint = { kind: "lessThan" | "lessThanOrEqual" | "greaterThan"; left: ValueRef; right: ValueRef; message: string } | { kind: "powerOfTwo"; value: ValueRef; message: string };
export type ArgumentToken = { kind: "literal"; value: string } | { kind: "mode" } | { kind: "input"; inputId: string } | { kind: "output" } | { kind: "parameter"; parameterId: string };
export type OutputDefinition =
  | { kind: "singleFile"; fileType: CdpFileType; extension: string; nameSuffix: string }
  | { kind: "genericRoot"; fileType: CdpFileType; extension: string; nameSuffix: string; discovery: { prefixStyle: "root-number" | "root-number-padded"; mayProduceRemnant: boolean } }
